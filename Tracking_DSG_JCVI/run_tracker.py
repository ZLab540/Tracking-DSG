...
sys.path.insert(0, './MDArchitecture/module')
...
def run_mdnet(img_list, init_bbox, gt=None, savefig_dir='', display=False):
    ...
    # Init model
    model = MDNet_new(opts['model_path'])
    if opts['use_gpu']:
        model = model.cuda()
    ...
    for i in range(1,len(img_list)):
        ...
        sum_overlap += overlap_ratio(gt[i],result_bb[i])[0]
        average = sum_overlap / (i + 1)
...
def main_func(frame_set_name):
    parser = argparse.ArgumentParser()
    #parser.add_argument('-s', '--seq', default='ants1', help='input seq')
    parser.add_argument('-s', '--seq', default=frame_set_name, help='input seq')
    parser.add_argument('-j', '--json', default='', help='input json')
    #parser.add_argument('-m', '--mat', default='', help='input mat')
    parser.add_argument('-f', '--savefig',default='./save_fig', action='store_true')
    parser.add_argument('-d', '--display',default='display_fig' ,action='store_true')
...
if __name__ == '__main__':
    dataset = os.listdir('/Users/shuwei5/Desktop/Zizi_tracking/vot2019')
    for frame_set_name in dataset:
        try:
            main_func(frame_set_name)
